...
# -------------------------------
print("[init] Loading Whisper model (base)...")
...
def record_audio():
    """Continuously record small audio chunks and push to queue."""
    samplerate = 16000
...
def process_audio():
    """Continuously transcribe audio using Whisper."""
    while recording:
...
# Start Whisper background threads
...
mouse_commands = (
    "left click", "right click", "double click", "click and hold",
...
def perform_command_from_text(text):
    ...
    if current_mode == "command":
        ...
        if len(tokens) >= 2 and tokens[0] in ("press", "type"):
            target = tokens[1]
            if len(target)==1:
                pyautogui.press(target); print(f"[action] pressed {target}"); return
            words_to_nums = {"zero":"0","one":"1","two":"2","three":"3","four":"4","five":"5","six":"6","seven":"7","eight":"8","nine":"9","ten":"10"}
            if target in words_to_nums:
                pyautogui.press(words_to_nums[target]); print(f"[action] pressed {words_to_nums[target]}"); return
